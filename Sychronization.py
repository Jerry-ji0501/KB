...
class SYN(nn.Module):
    def __init__(self, input_dim, output_dim, dropout_rate,
        ...
        self.encoder = GraphTransformerEncoder(dropout_rate,
                                               ...
                                               ffn_size,
                                               num_layer)
        self.decoder = GraphTransformerDecoder(hidden_size, dropout_rate, num_in_degree, embed_dim, num_out_degree,
                                               num_decoder_layers, num_heads, ffn_size)
        self.p_net = PriorNetwork(embed_dim, num_heads, input_dim, output_dim)
        self.q_net = RecognitionNetwork(num_layers, embed_dim, hidden_size, num_heads, attention_prob_dropout_prob,
...
if __name__ == '__main__':
    ...
    train_acces, train_loss, train_loss1, train_loss2, train_loss_kl = train(model, train_dataloader, mae, critrion,
                                                                             device, batch_size=4, train_epoch=20,
                                                                             lr=1e-5)
