...
class GraphConvolution(nn.Module):
    '''A simple implementation of GCN Layer'''
    def __init__(self,input_dim,output_dim,bias =True):
        ...
        if bias:
            self.bias = Parameter(torch.Float(output_dim))
        else:
...
def data_loader(file):
    ...
    with open(file2,'r',encoding='utf-8') as f2, open(file3,'r',encoding='utf-8') as f3:
        line2 = f2.readlines()
        line3 = f3.readlines()
        for line in line2:
            line = line.strip().split()
            line = [re.sub("'","",s) for s in line]
            print(line)
            if len(line) !=2:
                continue
    ...
    with open(file1,'r',encoding='utf-8') as f1:
        ...
        for line in content:
            ...
            h_ = entity2id[triple[1]]#0
            t_ = entity2id[triple[2]]#1
            r_ = relation2id[triple[0]]#2
...
class TransE:
    def __init__(self, entity_set, relation_set, triple_list,filename,
    ...
    def train(self, epochs):
        ...
        for epoch in range(epochs):
            ...
            writer.add_scalar('loss/loss', self.loss, epoch)
        ...
        print("写入文件...")
        with codecs.open("entity_50dim1"+self.filename, "w") as f1:
            for e in self.entity.keys():
                f1.write(e + "\t")
                f1.write(str(list(self.entity[e])))
                f1.write("\n")

        with codecs.open("relation_50dim1"+self.filename, "w") as f2:
            for r in self.relation.keys():
                f2.write(r + "\t")
                f2.write(str(list(self.relation[r])))
                f2.write("\n")

        with codecs.open("loss"+self.filename, "w") as f3:
...
if __name__ == '__main__':
    ...
    writer = SummaryWriter('.log\\data1')

    transE = TransE(entity_set, relation_set, triple_list,filename='data2', embedding_dim=100, learning_rate=0.01, margin=1, L1=True)
    ...
    writer = SummaryWriter('.log\\1942')

    transE = TransE(entity_set, relation_set, triple_list,filename='1942', embedding_dim=100, learning_rate=0.01, margin=1, L1=True)
    transE.emb_initialize()
    transE.train(epochs=2000)
    ...
    writer = SummaryWriter('.log\\tj')

    transE = TransE(entity_set, relation_set, triple_list,filename='tj', embedding_dim=100, learning_rate=0.01, margin=1, L1=True)
    transE.emb_initialize()
    transE.train(epochs=2000)
    ...
    writer = SummaryWriter('.log\\tbh')

    transE = TransE(entity_set, relation_set, triple_list,filename='tbh', embedding_dim=100, learning_rate=0.01, margin=1, L1=True)
    transE.emb_initialize()
    transE.train(epochs=2000)