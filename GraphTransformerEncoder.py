'''
      ...
      The function of GraphTransformerEncoder take the embedding of KnowledgeGraph as input :
          KG_Node_Feature: (Node_Num,Embed_dim)
          KG_In_Degree:(Node_Num,1)
          KG_Out_Degree:(Node_Num,1)
          KG_spatial_pos:(Node_Num,Node_Num)
...
Input of GraphTransformereEncoder:
      KG_Node_Features: (NodeFeatures)
      In_Degree:(Graph-Features)
      Out_Degree:(Graph-Features)
      Spatial_pos:(take is as  the shortest path between nodes)
      Graph_attn_bias:[Optional]   I don't know how does it work 
      
Output of GraphTransformerEncoder:
      Graph_Contextual_vector:
      Graph_attn:
      
Composition:
KG_embed_vector+in_degree_embedding+out_degree_embedding->Graph_representation_vector
 TransformerEncoderLayer(Norm->Multi-Attention mechanism-> FeedForward Neual Network)
 We can add Spatial_Position_Embedding  and Edge_Embedding into the layer to enhance the representation of graph structure  
 Finally ,we can got the hidden_state of KG Graph  
