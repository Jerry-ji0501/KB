...
dir_list = os.listdir(paths+'/adjacent_matrix')
for cur_file in dir_list:
    path = os.path.join(paths+'/adjacent_matrix',cur_file)
...
P0,P1  = np.reshape(windows_ZPD[int(dimensional)][:, 0], [1, 1, -1]), np.reshape(windows_ZPD[int(dimensional)][:, 1], [1, 1, -1])
weight = np.abs(P1 - P0)
