...
class RecognitionNetwork(nn.Module):
    ...
    def forward(self, KG_embed_vector):
        hidden_state = self.gnn(KG_embed_vector)
...
class GATlayers(nn.Module):
    ...
    def forward(self, graph_vector, att_scores=None):
        attn_prob, graph_vector = self.att_layer(hidden_states=graph_vector, attention_prob=att_scores)
...
class SelfAttention(nn.Module):
    def __init__(self, hidden_size, num_heads, attention_prob_dropout_prob, output_attention=False,
                 keep_multi_head=False):
    ...
    def forward(self, hidden_states, attention_prob=None):
        ...
        context_layers = context_layers.view((4, 128, 128))  # (*new_context_layer_shape)
...
# graph_vector = gnn(graph_vector,att_scores) print(attn_prob.shape) print(graph_vector) model = RecognitionNetwork(
# num_layers,embed_dim,hidden_size,num_heads,attention_prob_dropout_prob,dropout_rate,input_dim,output_dim) z =
# model(graph_vector,att_scores) print(z)
